...
internal partial class Program
{
    private static async Task<int> Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            if (!int.TryParse(selStr, out var selIdx) || selIdx < 0 || selIdx > functions.Count)
            {
            ...
            if (caseMap.Cases.Count > 0)
            {
                ...
                Console.WriteLine("  [0] Zurück zur Funktionsauswahl");
                Console.Write("Bitte Fall-Nummer wählen (z.B. 1): ");
                var cStr = Console.ReadLine()?.Trim();
                if (!int.TryParse(cStr, out var cNum) || cNum < 0)
                {
                ...
                if (!caseMap.Cases.ContainsKey(cNum))
                {
                    Console.Error.WriteLine("Ungültiger Fall.");
                    continue;
                }
